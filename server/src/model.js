...
class Model {
  ...
  checkPawn(oldPosition, newPosition, board, color) {
    ...
    if ((color === "UPPER" && oldX === 1) || (color === "LOWER" && oldX === 6)) {
    ...
    if (newX === oldX + direction && Math.abs(newY - oldY) === 1) {
      ...
      let destination = board[newX][newY];
      // Determine the color of the piece in the destination square
      let destinationColor = destination === destination.toUpperCase() ? "UPPER" : "LOWER";
  ...
  checkKnight(oldPosition, newPosition, board, color) {
    ...
    const horizontalTwoVerticalOne = Math.abs(newX - oldX) === 2 && Math.abs(newY - oldY) === 1;
  
    // Check if the new position is two squares vertically and one square horizontally from the old position
    const verticalTwoHorizontalOne = Math.abs(newY - oldY) === 2 && Math.abs(newX - oldX) === 1;
...
checkBishop(oldPosition, newPosition, board, color) {
  ...
  for (let x = oldX + xDirection, y = oldY + yDirection; x !== newX; x += xDirection, y += yDirection) {
  ...
  const destinationColor = destination === destination.toUpperCase() ? "UPPER" : "LOWER";
  return !destination || destinationColor !== color;
}
  ...
  checkQueen(oldPosition, newPosition, board, color) {
    ...
    return this.checkRook(oldPosition, newPosition, board, color) || this.checkBishop(oldPosition, newPosition, board, color);
}
